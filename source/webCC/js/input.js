...
//2バイトコード系のEnterのときには使えるか？
function replaceReplaceMode(event){
	targElem = evtToElement(event);
	if (inputbuffer == targElem.value ) { return; };
}
...
var setInputColor = function(evt){
	if($('input.nextinput:focus').length > 0){
		if( $('input.nextinput:focus')[0].value != ''){
			...
			$('input.nextinput:focus').css('background-color',"rgba(255,255,255,0.6)" );
		}else{
...
var inputAddElement = function (){
	...
	for(var i = 0; i < inputElements.length ;i++){
		...
		addEvent('input' ,
		inputElements[i] ,
		function evtReplaceMode( evt ){
			return replaceReplaceMode( evt );
		}
		);
		addEvent('keypress' ,
				inputElements[i] ,
