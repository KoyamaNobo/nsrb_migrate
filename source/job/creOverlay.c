...
int overlay_rec_len = 72;	//書式オーバレイファイルの最大レコード長
int fdd_len = 35;			//定義FD使用領域長
int sdd_len = 29;			//定義FD使用領域長
int itd_len = 56;			//定義FD使用領域長
int caption_len = 8;		//書見出し値の長(桁：49～56)
int continue_len = 46;		//書見出し値の継続最大長(桁：11～56)
...
typedef struct overlay_data{
	char fdd[51];			//FD定義
	char sdd[51];			//SD定義
	char itd[100][423];		//IT定義
	int itd_cnt;			//IT定義件数
}OVERLAY_DATA;
int itd_len_max = 422;	//IT定義の最大長
int goverlay_data_max = 100;	//書式オーバレイファイルの定義ITの最大件数
...
PDF_DATA gpdf_data[500];	//書式オーバレイpdfファイルの内容(構造体実体)
int gpdf_data_max = 500;	//書式オーバレイpdfファイルの内容の最大件数
...
int main(int argc, char *argv[]){
	...
	//引数の数を確認(書式オーバレイID)
	if (argc < 2){
		fprintf(stderr, " Error C [%02d-%02d] %s : Option is nothing\n", 00, 01, local_server_time(strTime));
		exit(EXIT_FAILURE);
	}

	//引数の長さを確認(14バイト以内であること)
	if (strlen(argv[1]) > 14){
		fprintf(stderr, " Error C [%02d-%02d] %s : Option is too long\n", 00, 02, local_server_time(strTime));
		exit(EXIT_FAILURE);
	}
	...
	if (ret != 0){
		fprintf(stderr," Error C [%02d-%02d] %s : Overlay File is not exist[%s]\n", 00, 02, local_server_time(strTime), PR_overlayfile);
		exit(EXIT_FAILURE);
	}
...
printf(">> main     PR_basename : %s\n" , PR_basename);
	...
	printf(">> Overlay PDF maker program stop (status : %d) \n", ret);
...
int PR_Open(){
	...
	if (setjmp(PR_env)) {
		...
		gtermid = 1;
		HPDF_Free(pdf);
		fprintf(stderr," Error C [%02d-%02d]: pdf free \n", gfuncid, gtermid);
		return -1;
	}
	...
	if (!pdf) {
		//pdf実体の生成失敗
		gtermid = 2;
		fprintf(stderr," Error C [%02d-%02d]: cannot create PdfDoc object\n", gfuncid, gtermid);
		return -2;
	}
	...
	if (strcmp(PR_id, "999") != 0){
		...
		if (strstr(PR_tempname, "XXXXXX") != NULL){
			if(mkstemp(PR_tempname) == -1){
				gtermid = 1;
				fprintf(stderr," Error C [%02d-%02d]: don't create temp File(%s) \n", gfuncid, gtermid, PR_tempname);
				ret = -1;
			}else{
				...
				if (ret != 0){
					...
					gtermid = 2;
					fprintf(stderr," Error C [%02d-%02d]: cannot authorize to temp File(%s) \n", gfuncid, gtermid, PR_tempname);
					ret = -1;
				}
			}
		}
		...
		if (ret != 0){
			gtermid = 3;
			fprintf(stderr," Error C [%02d-%02d]: cannot join Overlay PDF File(%s) \n", gfuncid, gtermid, PR_overlaypdf);
			return -1;
		}
		...
		if(ret == 0){
			...
			if (ret != 0){
				gtermid = 4;
				fprintf(stderr," Error C [%02d-%02d]: cannot delete temp File(%s) \n", gfuncid, gtermid, PR_tempname);
				return -1;
			}
		}
	}
...
printf(">>           dbHost : %s\n" , PR_db_server);
	...
	if (PR_maxPage == 0){
		gtermid = 1;
		fprintf(stderr, " Error C [%02d-%02d] : Pagemax value has not been set\n", gfuncid, gtermid);
		return 1;
	}
	return 0;
}
...
//	mybacktrace();
	...
	if (!mysql_real_connect(&PR_conn, PR_db_server, PR_db_user, PR_db_password, PR_db_database, PR_db_port, NULL, 0)) {  
		fprintf(stderr, " Error C [%02d-%02d]:db_connect server :%s user:%s pass:%s database:%s \n", gfuncid, gtermid, PR_db_server,PR_db_user,PR_db_password,PR_db_database);
		return 1;
	}
...
printf(">> PR_setProperty         PR_linepitch/PR_charpitch : %f / %f\n" , PR_linepitch, PR_charpitch);
	...
	if (strlen(PR_basename) == 0){
		gtermid = 2;
		fprintf(stderr," Error C [%02d-%02d]: cannot read database for PDF \n", gfuncid, gtermid);
		return -2;
	}
...
int PR_putInitoverlaypdf(){
	...
	if (ret != 0){
		gtermid = 1;
		fprintf(stderr," Error C [%02d-%02d]: cannot create Overlay PDF File(%s) \n", gfuncid, gtermid, PR_overlaypdf);
		return -1;
	}
	...
	if (ret != 0){
		...
		gtermid = 2;
		fprintf(stderr," Error C [%02d-%02d]: cannot authorize to Overlay PDF File(%s) \n", gfuncid, gtermid, PR_overlaypdf);
		return -2;
	}
			...
			if (strcmp(getStr(sbuf, 6, 2), "FD") == 0){
				...
				if (strcmp(getStr(sbuf, 10, 1), "-") == 0){
					...
					if (cap_len + strlen(cbuf) > text_len){
						gtermid = 1;
						fprintf(stderr," Error C [%02d-%02d]: Caption length over length=%s (%s)\n", gfuncid, gtermid, text_len, sbuf);
						ret = 1;
						break;
					}
					cap_len += strlen(cbuf);
...
printf(">> PR_edtFileID  str_bot : %s\n" , str_bot);
	...
	sprintf(PR_overlayfile, "%s%s%s", overlaypath, toLower(str_bot), txtChar);
	//書式オーバレイpdfファイルID
...
HPDF_REAL getColpos(HPDF_Point basPos, int Col, HPDF_REAL charWidth){
	...
	//微調整：1/2文字分前へ
	Colpos = basPos.x + (float)Col * charWidth - (charWidth / 2);
	//微調整：1/3文字分前へ
	Colpos -= charWidth / 3;
...
HPDF_REAL getRowpos(HPDF_Point basPos, int Row, HPDF_REAL lineHeight, int drawline, int centerline){
	...
	//微調整：1/2行分上へ
	Rowpos = basPos.y - (float)Row * (lineHeight * 2) + lineHeight;
...
char *toLower(char *str){
	...
	memset(gstrup, '\0', sizeof(gstrup));
	strcpy(gstrup, str);
	for (pos = 0; pos < strlen(gstrup); pos++){
		if (gstrup[pos] >= 'A' && gstrup[pos] <= 'Z'){
			//大文字のとき数置換で小文字に置き換え
			gstrup[pos] += 0x20;
		}
	}
