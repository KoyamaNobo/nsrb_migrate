       ...
       01  WN-AREA.
           ...
           02  WN-WANA   REDEFINES WN-WNAME.
             03  WN-WNAD   OCCURS  26.
               04  WN-WNA     PIC  N(001).
           02  WN-ONAME       PIC  N(014).
           02  WN-AONA  REDEFINES WN-ONAME.
             03  WN-ONAD  OCCURS  14.
               04  WN-ONA     PIC  N(001).
           02  WN-UNAME       PIC  N(014).
           02  WN-AUNA  REDEFINES WN-UNAME.
             03  WN-UNAD  OCCURS  14.
               04  WN-UNA     PIC  N(001).
           02  WN-KUM         PIC  N(004).
      ...
      *DISP-AREA1
       ...
       CALL "SD_Init" USING 
            "DSP-RVE" "RX" "1" "20" "12" " " "DISP-AREA1"
            RETURNING RESU.
       CALL "SD_Init" USING 
            "DISP-01" "N" "1" "21" "10" "DSP-RVE" " " RETURNING RESU.
       CALL "SD_Init" USING 
            "DISP-02" "N" "1" "9" "10" "DISP-01" " " RETURNING RESU.
       CALL "SD_Init" USING 
      ...
      *CLR-AREA
       ...
       CALL "SD_Init" USING 
            "CLR-01" "X" "5" "46" "1" " " "CLR-AREA" RETURNING RESU.
       CALL "SD_Init" USING 
      ...
      *********************************************
       ...
       MAIN-30.
           ...
           CALL "SD_Accept" USING BY REFERENCE ACT-03 "ACT-03" "9" "1"
            BY REFERENCE ESTAT RETURNING RESU.
           CALL "SD_Output" USING
            "DISP-MSG-SPACE" DISP-MSG-SPACE "p" RETURNING RESU.
           IF  ESTAT = "P9"
      ...
      ******************************
       TEST-RTN.
           ...
           END-IF
           MOVE       SPACE    TO        P-R.
           MOVE        X"1A24212474"  TO               CODEK1.
           MOVE       99       TO  R1-01 R1-02 R1-03.
           CALL "PR_LineFeed" USING "4" RETURNING RESP.
           CALL "PR_Write" USING P-R RETURNING RESP.
           MOVE       SPACE    TO        P-R.
      *
           MOVE        X"1A24212078"  TO               CODEK2.
           MOVE       SPACE    TO        R2-01.
           MOVE      "ÇmÇmÇmÇmÇmÇmÇmÇmÇm"    TO        R2-01.
           CALL "PR_LineFeed" USING "2" RETURNING RESP.
           CALL "PR_Write" USING P-R RETURNING RESP.
           MOVE       SPACE    TO        P-R.
      *
           MOVE        X"1A24212078"  TO               CODEK3.
           MOVE       SPACE    TO        R3-01.
           MOVE  ALL "Çm"    TO        F2.
           MOVE       SOM      TO        R3-01.
           CALL "PR_LineFeed" USING "2" RETURNING RESP.
      ...
      *
           MOVE        X"1A24212078"  TO              CODEK5.
           MOVE       SPACE    TO        R5-01  R5-02.
           MOVE  ALL "Çm"    TO        R5-01  R5-02.
           CALL "PR_LineFeed" USING "1" RETURNING RESP.
           CALL "PR_Write" USING P-R RETURNING RESP.
           MOVE       SPACE    TO        P-R.
      *
           MOVE  ALL  "X"      TO        R6-01  R6-02.
           CALL "PR_LineFeed" USING "2" RETURNING RESP.
           CALL "PR_Write" USING P-R RETURNING RESP.
           MOVE       SPACE    TO        P-R.
      *
           MOVE        X"1A24212078"  TO              CODEK7A.
           MOVE       SPACE    TO        R7-01  R7-02.
           MOVE  ALL "Çm"    TO        R7-01.
           CALL "PR_LineFeed" USING "1" RETURNING RESP.
           CALL "PR_Write" USING P-R RETURNING RESP.
           MOVE       SPACE    TO        P-R.
      *
      ...
      *
           ...
           MOVE       SPACE    TO        R5-01  R5-02.
           IF  OKJF-04        =  1
               MOVE  "ÇRÇPÇUÇVÇSÇT" TO     R5-02
           ELSE
               IF  OKJF-04        =  7
                   MOVE  "ÇVÇOÇQÇOÇSÇR" TO     R5-02
               END-IF
           END-IF
      ...
      *
           ...
           ELSE
               ...
               ELSE
                   IF  OKJF-04    =   4
                   MOVE      "óºîıâ^óAíÜélçëï®ó¨ÉZÉìÉ^Å[ì‡"  TO  R5-02
                   ELSE
                       IF  OKJF-04    =   7
                   ...
                   MOVE      "Å@Å@êÖìáç`çëç€ï®ó¨ÉZÉìÉ^Å[ì‡"  TO  R5-02
                           ELSE
                   MOVE      "â™éRésç°ÇWíöñ⁄ÇPÇUî‘ÇPÇVçÜÅ@"  TO  R5-02
                           END-IF
                       END-IF
                   END-IF
               END-IF
           ...
           MOVE       SPACE    TO        P-R.
      *
           MOVE  TC-TEL        TO        R6-01.
           IF  (OKJF-05  NOT <  0459002  AND NOT >  0459999)
               MOVE  "0798-64-5111  " TO      R6-02
           ELSE
               IF  (OKJF-05  NOT <  0460002  AND NOT >  0460999)
                   MOVE  "0798-67-0810"  TO      R6-02
               ELSE
                   IF  (OKJF-05  NOT <  3966002  AND NOT >  3966999)
                       MOVE  "03-5373-6711"  TO      R6-02
                   ELSE
                       MOVE  "086-243-2456  " TO      R6-02
                   END-IF
               END-IF
           END-IF
           CALL "PR_LineFeed" USING "2" RETURNING RESP.
      ...
      *
           MOVE        X"1A24212078"  TO              CODEK7A.
           MOVE       SPACE    TO        R7-01  R7-02.
           MOVE       WN-UNAME TO        R7-01.
           IF  (OKJF-05  NOT <  0459002  AND NOT >  0459999)
               MOVE      "Å@Å@äîéÆâÔé–è„éRõâã∆Å@Å@Å@Å@"  TO  R7-02
           ELSE
               IF  (OKJF-05  NOT <  0460002  AND NOT >  0460999)
                   MOVE      "Å@Å@äîéÆâÔé–ÉEÉGÉÑÉ}Å@Å@Å@Å@"  TO  R7-02
               ELSE
                   IF  (OKJF-05  NOT <  3966002  AND NOT >  3966999)
                   MOVE      "Å@Å@ëΩî‰ó«äîéÆâÔé–Å@Å@Å@Å@Å@"  TO  R7-02
                   ELSE
                       IF  OKJF-04    =   4
                   MOVE      "ì˙êiÉSÉÄÅ@ëÅìáîzëóÉZÉìÉ^Å[Å@"  TO  R7-02
                       ELSE
                           IF  OKJF-04    =   7
                   MOVE      "ì˙êiÉSÉÄáäÅ@í√éRï®ó¨ÉZÉìÉ^Å["  TO  R7-02
                           ELSE
                               IF  OKJF-04    =   6
                   MOVE      "ì˙êiÉSÉÄáäÅ@ã ìáï®ó¨ÉZÉìÉ^Å["  TO  R7-02
                               ELSE
                   MOVE      "Å@Å@ì˙êiÉSÉÄäîéÆâÔé–Å@Å@Å@Å@"  TO  R7-02
                               END-IF
                           END-IF
                       END-IF
                   END-IF
      ...
      *
       ...
       NAM-330.
           ...
           IF  WN-CNT3 NOT = 5
               ...
               GO TO NAM-330
           END-IF
           IF  WN-KUM NOT = "äîéÆâÔé–" AND "óLå¿âÔé–"
               GO TO NAM-320
           END-IF
      *
           IF  WN-KUM = "äîéÆâÔé–"
               MOVE "áä" TO WN-NA(WN-CNT1)
           END-IF
           IF  WN-KUM = "óLå¿âÔé–"
               MOVE "áã" TO WN-NA(WN-CNT1)
           END-IF
           MOVE WN-CNT1 TO WN-CNT2.
           ADD 3 TO WN-CNT2.
       NAM-340.
